...
**Key Features**:
- **Session persistence**: Saves `session_id` to `/tmp/soma-sessions/<session-key>.json`
...
| `kill()` | Clear session |
| `resumeLast()` | Restore from canonical session file |
...
```
- Writes to `AUDIT_LOG_PATH` (default: `/tmp/soma-audit.log`)
...
| `/resume` | Resume current chat session | Loads from `/tmp/soma-sessions/<session-key>.json` |
...
| `AUDIT_LOG_PATH` | Audit log location | `/tmp/soma-audit.log` |
...
New session created
  ...
  └─ Saved to /tmp/soma-sessions/<session-key>.json
  ↓
...
| File | Purpose | Format |
|------|---------|--------|
| `/tmp/soma-sessions/*.json` | Session persistence | JSON |
...
```bash
cp launchagent/ai.2lab.soma.plist.template \
   ~/Library/LaunchAgents/ai.2lab.soma.plist
...
- Logs to `/tmp/soma-ts.{log,err}`

**Useful Aliases**:
```bash
alias cbot='launchctl list | grep ai.2lab.soma'
alias cbot-stop='launchctl bootout gui/$(id -u)/ai.2lab.soma'
alias cbot-start='launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/ai.2lab.soma.plist'
alias cbot-restart='launchctl kickstart -k gui/$(id -u)/ai.2lab.soma'
alias cbot-logs='tail -f /tmp/soma.log'
```
...
**Compilation**:
```bash
bun build --compile src/index.ts --outfile soma
```
...
**Check audit log**:
```bash
tail -f /tmp/soma-audit.log
```

**Check session file**:
```bash
ls /tmp/soma-sessions
cat /tmp/soma-sessions/<session-key>.json
```
...
*This document describes soma as of commit hash from 2026-01-14.*
