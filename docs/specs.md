...
**Key Features**:
- **Session persistence**: Saves `session_id` to `/tmp/soma-session.json`
...
| `/resume` | Resume last session | Loads from `/tmp/soma-session.json` |
...
New session created
  ...
  └─ Saved to /tmp/soma-session.json
  ↓
...
| File | Purpose | Format |
|------|---------|--------|
| `/tmp/soma-session.json` | Session persistence | JSON |
